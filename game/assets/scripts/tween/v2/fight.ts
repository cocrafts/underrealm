...
import { updatePlayers } from '../../util/attribute';
import { instantiatePrefab, playEffectSound } from '../../util/resources';
import { CardUnits } from '../../v2/CardUnits';
...
export const animateCardAttack = async (
	...
	return new Promise((resolve) => {
		...
		const back = node.getChildByPath('back');
		...
		const translate = tween(node)
			...
			.call(() => {
				node.getComponent(CardUnits).updateUnits();
